...
public class ChatGUI extends javax.swing.JFrame implements UIControl {
    ...
    private void btnEnviarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEnviarActionPerformed
        ...
        if ((msg.trim().length() > 0) && (lstLista.getSelectedIndex() != -1)) {
            ...
            try {
                this.protoController.send(nickTarget, msg);
            } catch (Exception e) {
                e.printStackTrace();
                JOptionPane.showMessageDialog(this,
                        e.getMessage(),"Erro ao enviar mensagem.",JOptionPane.ERROR_MESSAGE);
            }
     ...
     */
    public void writeMessage(String id, String mensagem, Boolean privateMsg) {
        try {
            if (privateMsg) {
                styledDoc.insertString(styledDoc.getLength(), "<" + id + "> -- Private message", greenStyle);
            } else {
    ...
    public void writeLocalMessage(String id, String mensagem) {
        try {
            mensagem = this.replaceEmojis(mensagem);
    ...
    /** Troca os atalhos de emoji na mensagem pela representação unicode ele
     * @param mensagem
     * @return mensagem com emojis
     */

    private String replaceEmojis(String mensagem) {
        int[] codepoints = {0x1F60A};
        String smile = new String(codepoints, 0, codepoints.length);
        codepoints[0] = 0x1F604;
        String smile2 = new String(codepoints, 0, codepoints.length);
        codepoints[0] = 0x1F60C;
        String smile3 = new String(codepoints, 0, codepoints.length);
        codepoints[0] = 0x1F61B;
        String tongue = new String(codepoints, 0, codepoints.length);
        codepoints[0] = 0x1F61C;
        String tongue2 = new String(codepoints, 0, codepoints.length);
        codepoints[0] = 0x2639;
        String sad = new String(codepoints, 0, codepoints.length);
        codepoints[0] = 0x1F61F;
        String sad2 = new String(codepoints, 0, codepoints.length);
        codepoints[0] = 0x1F626;
        String surprise = new String(codepoints, 0, codepoints.length);
        codepoints[0] = 0x1F628;
        String surprise2 = new String(codepoints, 0, codepoints.length);
        codepoints[0] = 0x1F62E;
        String wow = new String(codepoints, 0, codepoints.length);
        codepoints[0] = 0x1F632;
        String wow2 = new String(codepoints, 0, codepoints.length);

        String msg = mensagem;
        msg = msg.replace(":)", smile);
        msg = msg.replace("(:", smile3);
        msg = msg.replace(":D", smile2);
        msg = msg.replace(":p", tongue);
        msg = msg.replace(";p", tongue2);
        msg = msg.replace(":(", sad);
        msg = msg.replace("):", sad2);
        msg = msg.replace("d:", surprise);
        msg = msg.replace("D:", surprise2);
        msg = msg.replace(":o", wow);
        msg = msg.replace(":O", wow2);
        return msg;
    }
    ...
    public void update(Message m) {
        switch (m.getType()) {
            ...
            case 2:
                ...
                break;
            case 3:
                this.writeMessage(m.getSource(), m.getMessage(), false);
                break;
            case 4:
                this.writeMessage(m.getSource(), m.getMessage(), true);
                break;
            case 5:
